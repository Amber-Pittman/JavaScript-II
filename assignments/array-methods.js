/* A local community center is holding a fundraising 5k fun run and has invited 
...
const donations = (acc, donate) => {
    return acc + donate;
}

runners.forEach(function(athelete) {
    ticketPriceTotal.push(athelete.donation);
});

ticketPriceTotal = ticketPriceTotal.reduce(donations);

console.log(ticketPriceTotal);

// ==== Challenge 5: Be Creative ====
/* Now that you have used .forEach(), .map(), .filter(), and .reduce().  I want you to think of 
potential problems you could solve given the data set and the 5k fun run theme.  Try to create 
and then solve 3 unique problems using one or many of the array methods listed above. */
